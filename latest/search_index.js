...

{
    ...
    "title": "Tables",
    "category": "section",
    "text": "Tables are coordinates in a tabular format (essentially a matrix), optionally with named columns. They have various constructors, for direct construction and also for conversion from other types.import PGFPlotsX\nconst pgf = PGFPlotsX\nusing LaTeXStringsimport PGFPlotsX\nconst pgf = PGFPlotsX\nusing LaTeXStrings\nsavefigs = (figname, obj) -> begin\n    pgf.save(figname * \".pdf\", obj)\n    run(`pdf2svg $(figname * \".pdf\") $(figname * \".svg\")`)\n    pgf.save(figname * \".tex\", obj);\n    return nothing\nendLetx = linspace(0, 2*pi, 100)\ny = sin.(x)x = linspace(0, 2*pi, 100)\ny = sin.(x)You can pass these coordinates in unnamed columns:pgf.Plot(pgf.Table([x, y]); incremental = false)\nsavefigs(\"table-unnamed-columns\", ans) # hide[.pdf], [generated .tex](Image: )or named columns:pgf.Plot(pgf.Table([:x => x, :y => y]); incremental = false)\nsavefigs(\"table-named-columns\", ans) # hide[.pdf], [generated .tex](Image: )or rename using options:pgf.@pgf pgf.Plot(\n    {\n        x = \"a\",\n        y = \"b\",\n    },\n    pgf.Table([:a => x, :b => y]);\n    incremental = false)\nsavefigs(\"table-dict-rename\", ans) # hide[.pdf], [generated .tex](Image: )In the example below, we use a matrix of values with edge vectors, and omit the points outside the unit circle:x = linspace(-1, 1, 20)\nz = @. 1 - √(abs2(x) + abs2(x'))\nz[z .≤ 0] .= -Inf\n@pgf.pgf pgf.Axis(\n    {\n        colorbar,\n        \"colormap/jet\",\n        \"unbounded coords\" = \"jump\"\n    },\n    pgf.Plot3(\n        {\n            surf,\n            shader = \"flat\",\n        },\n        pgf.Table(x, x, z);\n        incremental = false\n    )\n)\nsavefigs(\"table-jump-3d\", ans) # hide[.pdf], [generated .tex](Image: )"
},
...

{
    ...
    "title": "Julia types",
    "category": "section",
    "text": "There is some support to directly use Julia objects from different popular packages in PGFPlotsX.jl. Examples of these are given here. All code is assumed to include the following:import PGFPlotsX\nconst pgf = PGFPlotsX\nusing LaTeXStringsimport PGFPlotsX\nconst pgf = PGFPlotsX\nusing LaTeXStrings\nsavefigs = (figname, obj) -> begin\n    pgf.save(figname * \".pdf\", obj)\n    run(`pdf2svg $(figname * \".pdf\") $(figname * \".svg\")`)\n    pgf.save(figname * \".tex\", obj);\n    return nothing\nend"
},
